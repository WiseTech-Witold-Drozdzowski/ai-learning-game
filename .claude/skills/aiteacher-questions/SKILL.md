...
description: Generate a set of study questions for the AI Teacher app and write them into a seed JSON file under dev/ai-teacher/data/<subject>/, then import them into SQLite. Use when the user wants a new question set / topic, or to add questions to an existing topic. Triggers "aiteacher-questions", "write questions for <topic>".
---
...
(`dev/ai-teacher/data/ai-teacher.db`) is the source of truth for all state
(answers, evaluations, follow-ups, flags). The JSON files at
`dev/ai-teacher/data/<subject>/<topic>.json` are **seed files**: they only carry
...
4. Import into SQLite: run `npm run import` in `dev/ai-teacher/`. It prints how many
